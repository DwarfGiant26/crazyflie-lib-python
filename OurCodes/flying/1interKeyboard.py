...
def logconf_callback_1(timestamp, data, logconf):
    ...
    data['global_wind_direction'] = WIND_DIRECTION
    data['relative_wind_direction'] = WIND_DIRECTION
        ...
        with SyncCrazyflie(URI2, cf=Crazyflie(rw_cache='./cache')) as scf2:
            ...
            with SyncCrazyflie(URI3, cf=Crazyflie(rw_cache='./cache')) as scf3:
                ...
                for y in range(hover_time):
                    ...
                    time.sleep(0.1)
                print(f'go {second_drone_id} go to {destination[second_drone_id][0], destination[second_drone_id][1], destination[second_drone_id][2]+hi_relative_height}')
                print(f'go {third_drone_id} go to {intermediate[0], intermediate[1], intermediate[2]+hi_relative_height}')
                #go
                for y in range(hover_time):
